...
/**
 * Efficient quickselect algorithm for finding top-k maximum values
...
UserRec *get_similar_users(float *similarity_matrix, int user_id, int k,
  ...
  if (k > num_users - 1) k = num_users - 1;  // Exclude self
  ...
  // If we excluded self and need more results
  if (result_idx < k) {
...
ItemRec *get_item_recommendations(float *similarity_matrix,
  ...
  // Compute predicted ratings for each item
...
// Collaborative filtering: weighted average of similar users' ratings
#pragma omp parallel for collapse(2)
  for (int n = 0; n < num_neighbors; n++) {
    for (int item = 0; item < num_items; item++) {
      ...
      if (rating > 0.0f) {  // Only consider rated items
#pragma omp atomic
...
#pragma omp parallel for schedule(static)
  ...
  // Use quickselect for efficient top-k
